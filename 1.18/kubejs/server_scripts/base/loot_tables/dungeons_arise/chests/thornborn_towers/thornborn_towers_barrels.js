onEvent('server.datapack.high_priority', event => {
    let loot_table = {
        type: "minecraft:chest",
        pools: [{
                rolls: 1,
                entries: [{
                        type: "minecraft:item",
                        name: "minecraft:book",
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 8,
                                max: 14
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:enderman_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 3
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:emerald_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 3,
                                max: 5
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:experience_bottle",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 4,
                                max: 12
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:ender_eye"
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:gold_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 3
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 3,
                        name: "mysticalagriculture:iron_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 6
                            }
                        }]
                    }
                ]
            },
            {
                rolls: 1,
                entries: [{
                        type: "minecraft:item",
                        name: "minecraft:bread",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 16
                            }
                        }]
                    },
                    {
                        type: "minecraft:empty"
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:honey_bottle",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 8
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:honey_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 8
                            }
                        }]
                    }
                ]
            },
            {
                rolls: 1,
                entries: [{
                        type: "minecraft:item",
                        name: "minecraft:rose_bush",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:blue_orchid",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 2
                            }
                        }]
                    }
                ]
            },
            {
                rolls: {
                    min: 1,
                    max: 2
                },
                entries: [{
                        type: "minecraft:item",
                        name: "minecraft:golden_apple"
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:chainmail_helmet",
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 5,
                                max: 10
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:chainmail_boots",
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 5,
                                max: 10
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:chainmail_leggings",
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 5,
                                max: 10
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:chainmail_chestplate",
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 5,
                                max: 10
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:crossbow",
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 5,
                                max: 10
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:coal_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 8,
                                max: 16
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:book",
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 5,
                                max: 20
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 15,
                        name: "mysticalagriculture:lapis_lazuli_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 16
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 15,
                        name: "minecraft:book",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 4
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 8,
                        name: "mysticalagriculture:goldemerald_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 3,
                                max: 5
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 5,
                        name: "mysticalagriculture:emerald_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 4
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 10,
                        name: "mysticalagriculture:slime_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 4
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 15,
                        name: "mysticalagriculture:iron_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 6,
                                max: 14
                            }
                        }]
                    }
                ]
            }
        ]
    };
    event.addJson(`dungeons_arise:loot_tables/chests/thornborn_towers/thornborn_towers_barrels.json`, loot_table);;
});