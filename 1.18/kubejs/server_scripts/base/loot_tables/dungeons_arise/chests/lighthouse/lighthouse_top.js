onEvent('server.datapack.high_priority', event => {
    let loot_table = {
        type: "minecraft:chest",
        pools: [{
                rolls: {
                    min: 2,
                    max: 3
                },
                entries: [{
                        type: "minecraft:item",
                        name: "minecraft:nether_wart",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:music_disc_11"
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:skeleton_skull"
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:book",
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 1,
                                max: 12
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:tnt",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 4
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:redstone_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 12
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:glowstone_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 6
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:prismarine_shard",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 4,
                                max: 8
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:sponge"
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:iron_chestplate"
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:iron_helmet"
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:chorus_fruit",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:poppy",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:creeper_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 6
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:blue_orchid",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 2
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 5,
                        name: "minecraft:gold_nugget",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 4,
                                max: 12
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 15,
                        name: "mysticalagriculture:gold_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3
                            }
                        }]
                    },
                    {
                        type: "minecraft:item",
                        weight: 20,
                        name: "mysticalagriculture:iron_essence",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 4
                            }
                        }]
                    }
                ]
            },
            {
                rolls: 1,
                entries: [{
                        type: "minecraft:item",
                        name: "minecraft:map",
                        functions: [{
                            function: "minecraft:exploration_map",
                            destination: "dungeons_arise:shiraz_palace",
                            decoration: "target_x",
                            zoom: 4,
                            skip_existing_chunks: false
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "mysticalagriculture:stone_essence"
                    }
                ]
            }
        ]
    }
    event.addJson(`dungeons_arise:loot_tables/chests/infested_temple/infested_temple_room_bookshelf.json`, loot_table);
})