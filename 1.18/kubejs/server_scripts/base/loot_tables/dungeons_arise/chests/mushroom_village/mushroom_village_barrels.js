onEvent('server.datapack.high_priority', event => {
    let loot_table = {
    type: "minecraft:chest",
    pools: [{
            rolls: {
                min: 1,
                max: 2
            },
            entries: [{
                    type: "minecraft:item",
                    name: "minecraft:iron_sword",
                    functions: [{
                        function: "minecraft:enchant_with_levels",
                        levels: {
                            min: 4,
                            max: 12
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "minecraft:suspicious_stew",
                    functions: [{
                        function: "minecraft:set_stew_effect",
                        effects: [{
                            type: "minecraft:levitation",
                            duration: 240
                        }]
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "minecraft:chainmail_chestplate",
                    functions: [{
                        function: "minecraft:enchant_with_levels",
                        levels: 15
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "minecraft:diamond_sword",
                    functions: [{
                        function: "minecraft:enchant_with_levels",
                        levels: 5,
                        conditions: [{
                            condition: "minecraft:random_chance",
                            chance: 0.4
                        }]
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "minecraft:book",
                    functions: [{
                        function: "minecraft:enchant_with_levels",
                        levels: {
                            min: 0,
                            max: 16
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "mysticalagriculture:gold_essence",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 2,
                            max: 12
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "mysticalagriculture:iron_essence",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 4,
                            max: 16
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "minecraft:gold_ore",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 1,
                            max: 4
                        }
                    }]
                }
            ]
        },
        {
            rolls: 2,
            entries: [{
                    type: "minecraft:item",
                    name: "minecraft:potion",
                    functions: [{
                        function: "minecraft:set_nbt",
                        tag: "{display:{Name:'{\"text\":\"Fine Wine\"}'},CustomPotionEffects:[{Id:9b,Amplifier:0b,Duration:200},{Id:10b,Amplifier:3b,Duration:80}]}"
                    }]
                },
                {
                    type: "minecraft:tag",
                    weight: 100,
                    name: "minecraft:fishes",
                    expand: true,
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 12,
                            max: 16
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "minecraft:chorus_fruit"
                },
                {
                    type: "minecraft:item",
                    weight: 80,
                    name: "minecraft:milk_bucket",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 1,
                            max: 8
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    name: "minecraft:wet_sponge"
                },
                {
                    type: "minecraft:item",
                    weight: 50,
                    name: "minecraft:golden_apple"
                },
                {
                    type: "minecraft:item",
                    weight: 50,
                    name: "minecraft:golden_carrot"
                },
                {
                    type: "minecraft:item",
                    weight: 50,
                    name: "minecraft:mushroom_stew",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 2,
                            max: 3
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    weight: 100,
                    name: "minecraft:brown_mushroom",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 1,
                            max: 6
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    weight: 100,
                    name: "minecraft:red_mushroom",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 1,
                            max: 6
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    weight: 150,
                    name: "minecraft:honeycomb",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 16,
                            max: 32
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    weight: 100,
                    name: "minecraft:honey_bottle",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 8,
                            max: 16
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    weight: 200,
                    name: "minecraft:potato",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 1,
                            max: 16
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    weight: 200,
                    name: "minecraft:baked_potato",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 16,
                            max: 48
                        }
                    }]
                },
                {
                    type: "minecraft:item",
                    weight: 250,
                    name: "minecraft:bread",
                    functions: [{
                        function: "minecraft:set_count",
                        count: {
                            min: 14,
                            max: 16
                        }
                    }]
                }
            ]
        },
        {
            rolls: 1,
            entries: [{
                    type: "minecraft:loot_table",
                    name: "minecraft:chests/village/village_fisher"
                },
                {
                    type: "minecraft:loot_table",
                    name: "minecraft:chests/village/village_butcher"
                }]
            }
        ]
    }
    event.addJson(`dungeons_arise:loot_tables/chests/mushroom_village/mushroom_village_barrels.json`, loot_table);
})