onEvent('server.datapack.high_priority', event => {
    let loot_table = {
        type: "minecraft:chest",
        pools: [{
                rolls: {
                    min: 2,
                    max: 4,
                    type: "minecraft:uniform"
                },
                entries: [{
                        name: "minecraft:elytra",
                        type: "minecraft:item",
                        weight: 1
                    },
                    {
                        name: "minecraft:enchanted_golden_apple",
                        type: "minecraft:item",
                        weight: 5
                    },
                    {
                        name: "minecraft:end_crystal",
                        type: "minecraft:item",
                        weight: 9
                    },
                    {
                        name: "minecraft:shulker_shell",
                        type: "minecraft:item",
                        weight: 40
                    },
                    {
                        name: "minecraft:phantom_membrane",
                        type: "minecraft:item",
                        weight: 40,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 2,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:experience_bottle",
                        type: "minecraft:item",
                        weight: 65,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 2,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:obsidian_essence",
                        type: "minecraft:item",
                        weight: 65,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 6,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:purpur_block",
                        type: "minecraft:item",
                        weight: 65,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 6,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:diamond_essence",
                        type: "minecraft:item",
                        weight: 105,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 8,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:emerald_essence",
                        type: "minecraft:item",
                        weight: 105,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 3,
                                max: 8,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        type: "minecraft:empty",
                        weight: 145
                    },
                    {
                        name: "minecraft:book",
                        type: "minecraft:item",
                        weight: 165,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 25,
                                max: 45,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    }
                ]
            },
            {
                rolls: 4,
                entries: [{
                        name: "minecraft:purple_dye",
                        type: "minecraft:item",
                        weight: 10,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 8,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:enderman_essence",
                        type: "minecraft:item",
                        weight: 15,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 12,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:chorus_fruit",
                        type: "minecraft:item",
                        weight: 10,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 4,
                                max: 16,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:ender_eye",
                        type: "minecraft:item",
                        weight: 10,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 8,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:obsidian_essence",
                        type: "minecraft:item",
                        weight: 10,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 4,
                                max: 16,
                                type: "minecraft:uniform"
                            }
                        }]
                    }
                ]
            },
            {
                rolls: 1,
                entries: [{
                        type: "minecraft:empty",
                        weight: 20
                    },
                    {
                        name: "minecraft:ender_eye",
                        type: "minecraft:item",
                        weight: 20,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:map",
                        type: "minecraft:item",
                        weight: 1,
                        functions: [{
                            function: "minecraft:exploration_map",
                            destination: "repurposed_structures:stronghold_end",
                            decoration: "banner_black",
                            zoom: 1,
                            skip_existing_chunks: false
                        }]
                    }
                ]
            },
            {
                rolls: 0,
                bonus_rolls: 1,
                entries: [{
                        name: "minecraft:pink_banner",
                        type: "minecraft:item",
                        weight: 1,
                        functions: [{
                            function: "set_nbt",
                            tag: "{BlockEntityTag:{Patterns:[{Pattern:cs,Color:0},{Pattern:cs,Color:0},{Pattern:cs,Color:0},{Pattern:bo,Color:3},{Pattern:bo,Color:3},{Pattern:bo,Color:3}]}}"
                        }]
                    },
                    {
                        type: "minecraft:empty",
                        weight: 4
                    },
                    {
                        name: "minecraft:black_banner",
                        type: "minecraft:item",
                        weight: 4,
                        functions: [{
                            function: "set_nbt",
                            tag: "{BlockEntityTag:{Patterns:[{Pattern:bri,Color:10},{Pattern:bri,Color:15},{Pattern:ss,Color:15},{Pattern:flo,Color:10},{Pattern:flo,Color:15},{Pattern:gru,Color:10},{Pattern:bt,Color:10},{Pattern:bt,Color:10},{Pattern:bt,Color:15},{Pattern:bts,Color:6},{Pattern:bts,Color:15},{Pattern:gra,Color:15}]}}"
                        }]
                    },
                    {
                        name: "minecraft:experience_bottle",
                        type: "minecraft:item",
                        weight: 4,
                        functions: [{
                            function: "minecraft:set_count",
                            count: 2
                        }]
                    },
                    {
                        type: "minecraft:item",
                        name: "minecraft:potion",
                        weight: 4,
                        functions: [{
                            function: "minecraft:set_nbt",
                            tag: "{Potion:\"minecraft:levitation\"}"
                        }]
                    }
                ]
            }
        ]
    };
    event.addJson(`repurposed_structures:loot_tables/chests/pyramid/badlands_chest.json`, loot_table);;
})