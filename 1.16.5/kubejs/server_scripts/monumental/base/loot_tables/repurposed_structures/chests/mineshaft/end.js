onEvent('server.datapack.high_priority', event => {
    let loot_table = {
        type: "minecraft:chest",
        pools: [{
                rolls: {
                    min: 2,
                    max: 4,
                    type: "minecraft:uniform"
                },
                entries: [{
                        name: "minecraft:phantom_membrane",
                        type: "minecraft:item",
                        weight: 20
                    },
                    {
                        name: "minecraft:dragon_breath",
                        type: "minecraft:item",
                        weight: 2
                    },
                    {
                        name: "minecraft:experience_bottle",
                        type: "minecraft:item",
                        weight: 1
                    },
                    {
                        name: "minecraft:enchanted_golden_apple",
                        type: "minecraft:item",
                        weight: 1
                    },
                    {
                        name: "minecraft:name_tag",
                        type: "minecraft:item",
                        weight: 30
                    },
                    {
                        name: "minecraft:book",
                        type: "minecraft:item",
                        weight: 20,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 10,
                                max: 35,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:diamond_boots",
                        type: "minecraft:item",
                        weight: 2,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:diamond_chestplate",
                        type: "minecraft:item",
                        weight: 2,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:diamond_leggings",
                        type: "minecraft:item",
                        weight: 2,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:diamond_helmet",
                        type: "minecraft:item",
                        weight: 2,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:diamond_pickaxe",
                        type: "minecraft:item",
                        weight: 2,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:diamond_shovel",
                        type: "minecraft:item",
                        weight: 2,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:iron_boots",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:iron_chestplate",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:iron_leggings",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:iron_helmet",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:iron_pickaxe",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    },
                    {
                        name: "minecraft:iron_shovel",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_with_levels",
                            levels: {
                                min: 15,
                                max: 30,
                                type: "minecraft:uniform"
                            },
                            treasure: true
                        }]
                    }
                ]
            },
            {
                rolls: {
                    min: 2,
                    max: 4,
                    type: "minecraft:uniform"
                },
                entries: [{
                        name: "mysticalagriculture:diamond_essence",
                        type: "minecraft:item",
                        weight: 5,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 6,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:iron_essence",
                        type: "minecraft:item",
                        weight: 10,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 4,
                                max: 12,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:gold_essence",
                        type: "minecraft:item",
                        weight: 15,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 7,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:emerald_essence",
                        type: "minecraft:item",
                        weight: 5,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 6,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:popped_chorus_fruit",
                        type: "minecraft:item",
                        weight: 5,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:chorus_fruit",
                        type: "minecraft:item",
                        weight: 7,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 4,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:chorus_flower",
                        type: "minecraft:item",
                        weight: 12,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 4,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:chorus_plant",
                        type: "minecraft:item",
                        weight: 12,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 8,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:beetroot_seeds",
                        type: "minecraft:item",
                        weight: 7,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 4,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:purpur_block",
                        type: "minecraft:item",
                        weight: 5,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 4,
                                max: 10,
                                type: "minecraft:uniform"
                            }
                        }]
                    }
                ]
            },
            {
                rolls: 3,
                entries: [{
                        name: "minecraft:rail",
                        type: "minecraft:item",
                        weight: 20,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 4,
                                max: 8,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:powered_rail",
                        type: "minecraft:item",
                        weight: 5,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 4,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:detector_rail",
                        type: "minecraft:item",
                        weight: 5,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 4,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:activator_rail",
                        type: "minecraft:item",
                        weight: 5,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 4,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:end_rod",
                        type: "minecraft:item",
                        weight: 15,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 4,
                                type: "minecraft:uniform"
                            }
                        }]
                    }
                ]
            },
            {
                rolls: 0,
                bonus_rolls: 1,
                entries: [{
                        name: "minecraft:pink_banner",
                        type: "minecraft:item",
                        weight: 1,
                        functions: [{
                            function: "set_nbt",
                            tag: "{BlockEntityTag:{Patterns:[{Pattern:cs,Color:0},{Pattern:cs,Color:0},{Pattern:cs,Color:0},{Pattern:bo,Color:3},{Pattern:bo,Color:3},{Pattern:bo,Color:3}]}}"
                        }]
                    },
                    {
                        type: "minecraft:empty",
                        weight: 8
                    },
                    {
                        name: "minecraft:black_banner",
                        type: "minecraft:item",
                        weight: 4,
                        functions: [{
                            function: "set_nbt",
                            tag: "{BlockEntityTag:{Patterns:[{Pattern:gra,Color:2},{Pattern:gra,Color:2},{Pattern:gru,Color:2},{Pattern:gru,Color:10},{Pattern:gru,Color:10},{Pattern:sku,Color:15},{Pattern:sku,Color:2},{Pattern:sku,Color:10},{Pattern:cbo,Color:15},{Pattern:bo,Color:15},{Pattern:drs,Color:10},{Pattern:ss,Color:15}]}}"
                        }]
                    },
                    {
                        name: "minecraft:tnt",
                        type: "minecraft:item",
                        weight: 4,
                        functions: [{
                            function: "minecraft:set_count",
                            count: 2
                        }]
                    }
                ]
            }
        ]
    };
    event.addJson(`repurposed_structures:loot_tables/chests/mineshaft/end.json`, loot_table);;
});