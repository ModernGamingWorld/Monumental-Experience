onEvent('server.datapack.high_priority', event => {
    let loot_table = {
        type: "minecraft:chest",
        pools: [{
                rolls: {
                    min: 5,
                    max: 11,
                    type: "minecraft:uniform"
                },
                entries: [{
                        name: "minecraft:paper",
                        type: "minecraft:item",
                        weight: 8,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 12,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:chorus_fruit",
                        type: "minecraft:item",
                        weight: 7,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 2,
                                max: 12,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:beetroot_seeds",
                        type: "minecraft:item",
                        weight: 7,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 10,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:magenta_banner",
                        type: "minecraft:item",
                        weight: 7,
                        functions: [{
                                function: "minecraft:set_count",
                                count: {
                                    min: 2,
                                    max: 4,
                                    type: "minecraft:uniform"
                                }
                            },
                            {
                                function: "set_nbt",
                                tag: "{BlockEntityTag:{Patterns:[{Color:15,Pattern:tt}, {Color:15,Pattern:bt}]}}"
                            }
                        ]
                    },
                    {
                        name: "minecraft:popped_chorus_fruit",
                        type: "minecraft:item",
                        weight: 7,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 5,
                                max: 17,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:suspicious_stew",
                        type: "minecraft:item",
                        weight: 10,
                        functions: [{
                            function: "minecraft:set_stew_effect",
                            effects: [{
                                    type: "minecraft:blindness",
                                    duration: {
                                        min: 10,
                                        max: 25
                                    }
                                },
                                {
                                    type: "minecraft:levitation",
                                    duration: {
                                        min: 25,
                                        max: 55
                                    }
                                },
                                {
                                    type: "minecraft:weakness",
                                    duration: {
                                        min: 30,
                                        max: 50
                                    }
                                },
                                {
                                    type: "minecraft:jump_boost",
                                    duration: {
                                        min: 60,
                                        max: 120
                                    }
                                },
                                {
                                    type: "minecraft:night_vision",
                                    duration: {
                                        min: 60,
                                        max: 120
                                    }
                                },
                                {
                                    type: "minecraft:saturation",
                                    duration: {
                                        min: 10,
                                        max: 12
                                    }
                                },
                                {
                                    type: "minecraft:invisibility",
                                    duration: {
                                        min: 30,
                                        max: 120
                                    }
                                },
                                {
                                    type: "minecraft:slow_falling",
                                    duration: {
                                        min: 30,
                                        max: 120
                                    }
                                },
                                {
                                    type: "minecraft:glowing",
                                    duration: {
                                        min: 30,
                                        max: 120
                                    }
                                }
                            ]
                        }]
                    },
                    {
                        name: "minecraft:phantom_membrane",
                        type: "minecraft:item",
                        weight: 6,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:enderman_essence",
                        type: "minecraft:item",
                        weight: 5,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:end_rod",
                        type: "minecraft:item",
                        weight: 2,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:magenta_stained_glass_pane",
                        type: "minecraft:item",
                        weight: 2,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 3,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "mysticalagriculture:creeper_essence",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 10,
                                max: 15,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:firework_star",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                                function: "minecraft:set_count",
                                count: {
                                    min: 1,
                                    max: 3,
                                    type: "minecraft:uniform"
                                }
                            },
                            {
                                function: "set_nbt",
                                tag: "{Explosion:{Colors:[I;8073150], FadeColors:[I;12801229], Trail:1b, Flicker:1b, Type:2b}}"
                            }
                        ]
                    },
                    {
                        name: "minecraft:firework_star",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                                function: "minecraft:set_count",
                                count: {
                                    min: 1,
                                    max: 3,
                                    type: "minecraft:uniform"
                                }
                            },
                            {
                                function: "set_nbt",
                                tag: "{Explosion:{Colors:[I;12801229], FadeColors:[I;8073150], Trail:1b, Flicker:1b, Type:2b}}"
                            }
                        ]
                    },
                    {
                        name: "mysticalagriculture:obsidian_essence",
                        type: "minecraft:item",
                        functions: [{
                            function: "minecraft:set_count",
                            count: {
                                min: 1,
                                max: 6,
                                type: "minecraft:uniform"
                            }
                        }]
                    },
                    {
                        name: "minecraft:iron_helmet",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_randomly"
                        }]
                    },
                    {
                        name: "minecraft:iron_chestplate",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_randomly"
                        }]
                    },
                    {
                        name: "minecraft:iron_leggings",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_randomly"
                        }]
                    },
                    {
                        name: "minecraft:iron_boots",
                        type: "minecraft:item",
                        weight: 3,
                        functions: [{
                            function: "minecraft:enchant_randomly"
                        }]
                    }
                ]
            },
            {
                rolls: 0,
                bonus_rolls: 1,
                entries: [{
                        type: "minecraft:empty",
                        weight: 25
                    },
                    {
                        name: "minecraft:suspicious_stew",
                        type: "minecraft:item",
                        weight: 47,
                        functions: [{
                            function: "minecraft:set_stew_effect",
                            effects: [{
                                    type: "minecraft:blindness",
                                    duration: {
                                        min: 10,
                                        max: 25
                                    }
                                },
                                {
                                    type: "minecraft:levitation",
                                    duration: {
                                        min: 25,
                                        max: 55
                                    }
                                },
                                {
                                    type: "minecraft:weakness",
                                    duration: {
                                        min: 30,
                                        max: 50
                                    }
                                },
                                {
                                    type: "minecraft:jump_boost",
                                    duration: {
                                        min: 60,
                                        max: 120
                                    }
                                },
                                {
                                    type: "minecraft:night_vision",
                                    duration: {
                                        min: 60,
                                        max: 120
                                    }
                                },
                                {
                                    type: "minecraft:saturation",
                                    duration: {
                                        min: 10,
                                        max: 12
                                    }
                                },
                                {
                                    type: "minecraft:invisibility",
                                    duration: {
                                        min: 30,
                                        max: 120
                                    }
                                },
                                {
                                    type: "minecraft:slow_falling",
                                    duration: {
                                        min: 30,
                                        max: 120
                                    }
                                },
                                {
                                    type: "minecraft:glowing",
                                    duration: {
                                        min: 30,
                                        max: 120
                                    }
                                }
                            ]
                        }]
                    },
                    {
                        name: "minecraft:firework_star",
                        type: "minecraft:item",
                        weight: 14,
                        functions: [{
                                function: "minecraft:set_count",
                                count: 3
                            },
                            {
                                function: "set_nbt",
                                tag: "{Explosion:{Colors:[I;8073150], FadeColors:[I;12801229], Trail:1b, Flicker:1b, Type:2b}}"
                            }
                        ]
                    },
                    {
                        name: "minecraft:firework_star",
                        type: "minecraft:item",
                        weight: 14,
                        functions: [{
                                function: "minecraft:set_count",
                                count: 3
                            },
                            {
                                function: "set_nbt",
                                tag: "{Explosion:{Colors:[I;12801229], FadeColors:[I;8073150], Trail:1b, Flicker:1b, Type:2b}}"
                            }
                        ]
                    }
                ]
            }
        ]
    };
    event.addJson(`repurposed_structures:loot_tables/chests/shipwreck/end/supply_chest.json`, loot_table);;
})